...
export const POST: RequestHandler = async ({ params, request, locals }) => {
  try {
    ...
    // Notificar al creador de la encuesta si es un voto nuevo
    // (y si no es el mismo usuario que vota)
    if (!isUpdate && vote.userId !== option.poll.userId) {
      // Obtener título de la encuesta para la notificación si no lo tenemos completo (option.poll debe tenerlo)
      // Ya tenemos option.poll incluido arriba con true, así que tenemos el título.

      const pollTitle = option.poll.title;
      // Truncate si es muy largo
      const displayTitle = pollTitle.length > 50 ? pollTitle.substring(0, 50) + '...' : pollTitle;

      // Ejecutar fire-and-forget
      createNotification({
        userId: option.poll.userId,
      ...
      }).catch(err => console.error('[Vote Notification] Error ignoring:', err));
    }
