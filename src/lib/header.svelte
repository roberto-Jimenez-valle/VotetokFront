<script lang="ts">
			...
			try {
				...
				if (response.ok) {
					...
					for (const place of places) {
						const type =
							place.level === 1 ? "country" : "subdivision";
	...
	function handleNavigateToView(level: string) {
		...
		window.dispatchEvent(
			new CustomEvent("headerNavigateToView", { detail: { level } }),
		);
	}
...
<header
	class="top-0 left-0 right-0 z-50 pt-[env(safe-area-inset-top)]"
...
>
	<div class="w-full flex flex-col">
		...
		<div class="transition-opacity duration-300 ease-in-out px-2 sm:px-4">
			<div
				class="flex items-center justify-between h-14 sm:h-14 md:h-16 w-full"
			>
				<img
					src="/logo.png"
					alt="VouTop"
		...
		>
			<div class="avatars-scroll-container">
				<div class="header-nav-minimal" bind:this={navScrollContainer}>
					...
					{:else}
						...
						<button
							class="nav-chip"
							onclick={() => handleNavigateToView("world")}
						>
							Global
						</button>
					{/if}
					...
					{#if selectedCountryName}
						...
						{:else}
							<button
								class="nav-chip"
								onclick={() => handleNavigateToView("country")}
							>
								{selectedCountryName}
							</button>
						{/if}
					...
					{#if selectedSubdivisionName}
						...
						{:else}
							<button
								class="nav-chip"
								onclick={() =>
									handleNavigateToView("subdivision")}
							>
								{selectedSubdivisionName}
							</button>
						{/if}
				...
				{#if showSearch}
					<div class="nav-search-overlay">
						...
						<input
							...
							placeholder={$navigationState.level === "world"
								? "Buscar país..."
								: "Buscar región..."}
							bind:value={tagQuery}
						...
						<div class="nav-search-results">
							...
							{:else if searchResults.length > 0}
								{#each searchResults as result}
									<button
										class="search-result-item {result.hasData ===
										false
											? 'no-data'
											: ''}"
										onclick={() =>
										...
										<span class="result-name">
											{#if result.id && result.id.includes(".")}
												<span class="hierarchy">
													<span
														class="hierarchy-separator"
														>→</span
													>
													<span
														class="hierarchy-subdivision"
														>{result.parentName ||
															"Subdivisión"}</span
													>
													<span
			...
			<!-- cierre avatars-scroll-container -->
		</div>
		<!-- cierre avatars-scroll-wrapper -->
	</div>
	<!-- cierre w-full flex flex-col -->
</header>
...
<style>
	...
	.nav-chip:active {
		box-shadow:
			...
			inset -3px -3px 6px
				var(--neo-shadow-light, rgba(255, 255, 255, 0.6));
	}
	...
	.nav-search-clear-btn:active {
		box-shadow:
			...
			inset -2px -2px 4px
				var(--neo-shadow-light, rgba(255, 255, 255, 0.5));
	}
