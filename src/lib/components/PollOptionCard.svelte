<script lang="ts">
  ...
  interface Props {
    ...
    class?: string;
    showQuizGuide?: boolean;
    showCorrectOptionButton?: boolean;
  }
  ...
  $effect(() => {
    if (friends.length > 0) {
      console.log(
        "[PollOptionCard] friends received:",
        friends.length,
        "userHasVoted:",
        userHasVoted,
        "mode:",
  ...
  const isThumbnailPlatform = $derived(
    [
      "youtube",
      "vimeo",
      "dailymotion",
  ...
  const linkUrl = $derived(
    imageUrl && !imageUrl.match(/\.(jpg|jpeg|png|webp|gif|svg|bmp)(\?|$)/i)
      ? imageUrl
      : null,
  );
  ...
  function handleLabelInput(newText: string) {
    ...
    if (currentUrl) {
      ...
      const newLabel = newText.trim()
        ? `${newText.trim()} ${currentUrl}`
        : currentUrl;
  ...
  const displayColor = $derived(
    mode === "edit" || userHasVoted ? color : NEUTRAL_COLOR,
  );
  ...
  async function fetchRealThumbnail(
    url: string,
    type: string,
  ): Promise<string> {
    ...
    const fetchPromise = (async () => {
      try {
        ...
        if (response.ok) {
          ...
          console.warn(
            "[PollOptionCard] ❌ link-preview failed:",
            response.status,
          );
        }
      } catch (err) {
  ...
  $effect(() => {
    ...
    const needsThumbnail =
      (isVideoType || isThumbnailPlatform || showGenericLinkBadge) &&
      !isGifType &&
  ...
  const videoThumbnail = $derived(
    fetchedThumbnail || getDefaultThumbnail(mediaType),
  );
  ...
  // Estado local para controlar si se ha cerrado el tooltip de guía
  let quizGuideDismissed = $state(false);
</script>

<!-- svelte-ignore a11y_no_noninteractive_tabindex -->
<div
  class="poll-option-card {mode} {className}"
  ...
  onkeydown={(e) => {
    if (isClickable && mode === "view" && (e.key === "Enter" || e.key === " "))
      onClick?.(e);
  }}
>
  {#if isVideoType}
    ...
    <div class="card-video-wrapper" style="background-color: {displayColor};">
            ...
            <img
              ...
              onerror={(e) => {
                (e.target as HTMLImageElement).src =
                  "https://placehold.co/320x180/333/white?text=Video";
              }}
            />
            <div
              class="platform-icon-overlay"
              style="--platform-color: {platformColors[mediaType] || '#666'}"
            >
              ...
              {:else if mediaType === "bandcamp"}
                <svg viewBox="0 0 24 24" fill="currentColor"
                  ><path d="M0 18.75l7.437-13.5H24l-7.438 13.5H0z" /></svg
                >
              {:else}
        ...
        <div class="card-bottom-row">
          ...
          {#if friends.length > 0 && mode !== "edit"}
            <button
              ...
              onclick={(e) => {
                e.stopPropagation();
                if (userHasVoted) onFriendsClick?.();
              }}
                ...
                <div class="friend-avatar-wrapper" style="z-index: {10 - i};">
                  {#if userHasVoted}
                    <img
                      ...
                      shouldRetryImage(friend.avatarUrl)
                        ? friend.avatarUrl
                        : DEFAULT_AVATAR}
                      alt={friend.name}
                      class="friend-avatar"
                      loading="lazy"
                      onerror={(e: Event) => {
                        if (e.target && friend.avatarUrl) {
                          (e.target as HTMLImageElement).src = DEFAULT_AVATAR;
                          markImageFailed(friend.avatarUrl);
                        }
                      }}
                    />
                  {:else}
      ...
      {#if userHasVoted && isCorrect}
        <div class="correct-indicator-overlay">
          <div
            class="correct-indicator-badge {isVoted ? 'correct' : 'incorrect'}"
          >
            {#if isVoted}
              <Check size={14} />
              <span>¡Acertaste!</span>
          ...
          {#if isThumbnailPlatform && hasRealThumbnail && !thumbnailFailed && shouldRetryImage(videoThumbnail || "")}
            ...
            <img
              ...
              onerror={() => {
                ...
                markImageFailed(videoThumbnail || "");
              }}
            />
            <div
  ...
  {#if mode === "edit"}
    <div class="floating-actions-right">
      {#if showCorrectOptionButton}
        <div style="position: relative;">
          {#if showQuizGuide && !quizGuideDismissed}
            <div class="quiz-guide-tooltip" transition:fade={{ duration: 200 }}>
              <span
                style="margin-right: 16px; text-align: left; line-height: 1.2;"
                >Marca este icono para elegir<br />la opcion correcta de la
                encuesta</span
              >
              <button
                class="quiz-guide-close"
                onclick={(e) => {
                  e.stopPropagation();
                  quizGuideDismissed = true;
                }}
                type="button"
              ...
              <div class="quiz-guide-arrow"></div>
            </div>
          {/if}
          <button
            type="button"
            class="tool-btn {isCorrect ? 'active' : ''} {showQuizGuide
              ? 'blinking-highlight'
              : ''}"
            onclick={(e) => {
              e.stopPropagation();
...
<style>
  ...
  .quiz-guide-tooltip {
    ...
    font-size: 13px;
    font-weight: 600;
    /* white-space: nowrap; REMOVED to allow multiline */
    /* pointer-events: none; REMOVED to allow clicking close button */
    z-index: 50;
