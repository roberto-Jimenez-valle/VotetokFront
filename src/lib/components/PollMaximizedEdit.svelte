<script lang="ts">
  ...
  function getPreviewThumbnail(url: string): string {
    ...
    if (url.includes("spotify.com")) {
      return (
        "data:image/svg+xml," +
        encodeURIComponent(
          '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 100 100"><rect fill="#1DB954" width="100" height="100"/><circle cx="50" cy="50" r="40" fill="#000" opacity="0.3"/></svg>',
        )
      );
    }
    ...
    if (url.includes("soundcloud.com")) {
      return (
        "data:image/svg+xml," +
        encodeURIComponent(
          '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 100 100"><rect fill="#ff5500" width="100" height="100"/></svg>',
        )
      );
    }
    ...
    if (url.includes("tiktok.com")) {
      return (
        "data:image/svg+xml," +
        encodeURIComponent(
          '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 100 100"><rect fill="#000000" width="100" height="100"/></svg>',
        )
      );
    }
    ...
    return (
      "data:image/svg+xml," +
      encodeURIComponent(
        '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 100 100"><rect fill="#333" width="100" height="100"/><polygon points="40,30 40,70 70,50" fill="#fff" opacity="0.8"/></svg>',
      )
    );
  }
  ...
  let previewCache = $state<
    Record<string, { image: string; loading: boolean }>
  >({});
  ...
  function getPlaceholderForPlatform(url: string): string {
    ...
    if (lowerUrl.includes("instagram.com"))
      return "https://placehold.co/640x360/E1306C/white?text=%F0%9F%93%B7+Instagram";
  ...
  async function fetchPreviewThumbnail(optId: string, url: string) {
    ...
    if (
      previewCache[optId]?.image &&
      !previewCache[optId].loading &&
      !previewCache[optId].image.includes("placehold.co")
    ) {
      return;
    }
    ...
    if (vimeoMatch) {
      previewCache = {
        ...previewCache,
        [optId]: {
          image: `https://vumbnail.com/${vimeoMatch[1]}.jpg`,
          loading: false,
        },
      };
      return;
    ...
    try {
      ...
      if (response.ok) {
        ...
        previewCache = {
          ...previewCache,
          [optId]: { image: getPlaceholderForPlatform(url), loading: false },
        };
      }
    } catch (error) {
  ...
  $effect(() => {
    ...
    const optionsCopy = options.map((o) => ({
      id: o.id,
      imageUrl: o.imageUrl,
      label: o.label,
    }));
      ...
      <div class="poll-meta-info">
        ...
        >
          ...
          >
            ...
            <svg
              class="w-3 h-3 chevron-icon {showOptionsDropdown ? 'open' : ''}"
              fill="none"
              stroke="currentColor"
              viewBox="0 0 24 24"
            >
          ...
          {#if showOptionsDropdown}
            <!-- svelte-ignore a11y_no_static_element_interactions -->
            <div
              class="meta-dropdown options-dropdown-max"
            ...
            >
              ...
              {#each options as opt, i (opt.id)}
                ...
                <div
                  class="dropdown-option-row-max {draggedOptionIndex === i
                    ? 'dragging'
                ...
                >
                  ...
                  <button
                    type="button"
                    class="dropdown-option-max {activeIndex === i
                      ? 'active'
                  ...
                  >
                    <span
                      class="option-dot-max"
                      style="background-color: {opt.color};"
                    ></span>
                    <span class="option-num-max">{i + 1}</span>
                    <span class="option-text-max"
                      >{opt.label.trim() || `Opción ${i + 1}`}</span
                    >
                    {#if !hasText && !hasMedia}
                      <svg
                        class="w-3.5 h-3.5 warning-icon"
                        fill="none"
                        stroke="currentColor"
                        viewBox="0 0 24 24"
                      >
              ...
              {#if canAddOption}
                <button
                  type="button"
                  class="dropdown-option-max add-btn-max"
                  onclick={() => {
                    onAddOption?.();
                  }}
                >
        ...
        >
          ...
          >
            ...
            <svg
              class="w-3 h-3 chevron-icon {showDurationDropdown ? 'open' : ''}"
              fill="none"
              stroke="currentColor"
              viewBox="0 0 24 24"
            >
          ...
          {#if showDurationDropdown}
            <!-- svelte-ignore a11y_no_static_element_interactions -->
            <div
              class="meta-dropdown duration-dropdown-max"
              transition:fly={{ y: -5, duration: 150 }}
              onclick={(e) => e.stopPropagation()}
            >
              ...
              {#if !showCustomDatePicker}
                {#each durations as dur}
                  <button
                    type="button"
                    class="dropdown-option-max {duration === dur.value
                      ? 'active'
                      : ''}"
                    onclick={() => {
                  ...
                  >
                    <svg
                      class="w-4 h-4 duration-icon-max"
                      fill="none"
                      stroke="currentColor"
                      viewBox="0 0 24 24"
                    >
                ...
                <button
                  type="button"
                  class="dropdown-option-max custom-date-btn {isCustomDuration(
                    duration,
                  )
                    ? 'active'
                    : ''}"
            ...
            <div class="option-card-container">
              <div class="option-card-rounded">
                      ...
                      {#if isGenericLinkWithoutThumb && mediaUrl}
                        <div class="link-below-wrapper">
                          ...
                          >
                            ...
                            />
                            <span class="link-below-domain-edit"
                              >{getHostname(mediaUrl)}</span
                            >
                ...
                {:else if isVideoType}
                  ...
                  <div
                    class="card-video-wrapper {isMusicType ? 'is-music' : ''}"
                    class:has-yesno={opt.isYesNo}
                    style="background-color: {opt.color};"
                  >
                      ...
                      <div class="floating-preview-inner">
                        ...
                        {#if thumbUrl}
                          ...
                          >
                            <!-- Badge de plataforma -->
                            <div
                              class="platform-badge-edit"
                              style="--platform-color: {platformColors[type] ||
                                '#666'}"
                            >
                              {#if type === "youtube"}
                ...
                {:else}
                  ...
                  >
                    ...
                    <div
                      ...
                      role={!hasEmbeddableContent(mediaUrl)
                        ? "link"
                        : undefined}
                      style={!hasEmbeddableContent(mediaUrl)
                        ? "cursor: pointer;"
                        : ""}
                    >
                      <!-- Imagen de fondo -->
                ...
                <div class="floating-actions-right">
                  ...
                  <button
                    type="button"
                    class="tool-btn {opt.isCorrect || opt.correctAnswer
                      ? 'active'
                      : ''} {pollType === 'quiz' ? 'quiz-mode-btn' : ''}"
                    onclick={(e) => {
                      e.stopPropagation();
                      if (pollType === "quiz") {
                        // Solo una correcta en quiz
                        options.forEach((o) => (o.isCorrect = false));
                        options = [...options];
                        onToggleCorrect?.(opt.id);
                      } else {
                        onToggleCorrect?.(opt.id);
                      }
                    }}
                    aria-label="Marcar correcta"
                    title={opt.isCorrect || opt.correctAnswer
                      ? "Opción correcta"
                      : "Marcar como correcta"}
                  >
                    <CircleCheck size={18} />
                    {#if showQuizHint && pollType === "quiz" && i === activeIndex}
                      <div
                        class="quiz-hint-tooltip-max"
                  ...
                  <div class="yesno-overlay">
                    ...
                    {:else if opt.correctAnswer}
                      <div class="yesno-correct-label">
                        <CircleCheck size={10} />
                        <span
                          >"{opt.correctAnswer === "yes"
                            ? opt.yesText || "Sí"
                            : opt.noText || "No"}" es correcta</span
                        >
                      </div>
                    {/if}
                    ...
                    <div class="yesno-row-max">
                      <button
                        type="button"
                        class="option-btn-max {opt.correctAnswer === 'yes'
                          ? 'selected'
                          : ''}"
                        onclick={(e) => {
                          e.stopPropagation();
                          onToggleCorrect?.(opt.id, "yes");
                        }}
                      >
                        {#if opt.isCorrect}
                          <div
                            class="option-selector-max {opt.correctAnswer ===
                            'yes'
                              ? 'selected'
                              : ''}"
                          >
                      ...
                      <button
                        type="button"
                        class="option-btn-max {opt.correctAnswer === 'no'
                          ? 'selected'
                          : ''}"
                        onclick={(e) => {
                          e.stopPropagation();
                          onToggleCorrect?.(opt.id, "no");
                        }}
                      >
                        {#if opt.isCorrect}
                          <div
      ...
      <div class="poll-types-buttons">
        {#each pollTypes as type}
          <button
            ...
            style={pollType === type.id
              ? `background-color: ${type.color}; border-color: ${type.color}; color: white;`
              : `color: ${type.color};`}
            onclick={() => {
              onChangePollType?.(type.id);
              // onOpenTypeOptionsModal?.(); // Modal desactivado

              if (type.id === "quiz") {
                showQuizHint = true;
                setTimeout(() => (showQuizHint = false), 3000);
              }
            }}
