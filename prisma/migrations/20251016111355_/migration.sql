/*
  ...
  - You are about to drop the column `city_name` on the `votes` table. All the data in the column will be lost.
  - You are about to drop the column `country_iso3` on the `votes` table. All the data in the column will be lost.
  - You are about to drop the column `country_name` on the `votes` table. All the data in the column will be lost.
  - Made the column `subdivision_id` on table `votes` required. This step will fail if there are existing NULL values in that column.
...
INSERT INTO "new_votes" ("created_at", "id", "ip_address", "latitude", "longitude", "option_id", "poll_id", "subdivision_id", "subdivision_name", "user_agent", "user_id") SELECT "created_at", "id", "ip_address", "latitude", "longitude", "option_id", "poll_id", "subdivision_id", "subdivision_name", "user_agent", "user_id" FROM "votes";
DROP TABLE "votes";
