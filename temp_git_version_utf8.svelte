<script lang="ts">
  ...
  function handleModalDragEnd() {
    ...
    // Si se arrastr├│ m├ís de 100px hacia abajo, cerrar el modal
  ...
  // No inicializar autom├íticamente ninguna opci├│n como activa
  // Se activar├í solo cuando el usuario haga scroll
  ...
  // Detectar si el scroll est├í al final o al principio
  function checkScrollPosition() {
    ...
    // Detectar qu├® opci├│n est├í m├ís visible (m├ís cerca del centro)
    if (showAllOptions) {
    ...
    if (moveDistance < 10) {
      ...
      if (timeSinceLastTap < 300 && timeSinceLastTap > 0) {
        ...
        if (activeOption && readOnly) {
          console.log('[PollMaximizedView] ­ƒù│´©Å Doble tap - Votando:', activeOption.id);
      ...
      if (Math.abs(deltaX) > Math.abs(deltaY)) {
        ...
        if (Math.abs(deltaX) > 50) {
          ...
          setTimeout(() => {
            if (deltaX < 0) {
              // Swipe a la izquierda ÔåÆ siguiente (sale hacia izquierda)
              transitionX = 300;
              transitionY = 0;
              goToNext();
              setTimeout(() => showVoteBorder = true, 500); // 250ms out + 250ms in
            } else {
              // Swipe a la derecha ÔåÆ anterior (sale hacia derecha)
              transitionX = -300;
              transitionY = 0;
              goToPrevious();
              setTimeout(() => showVoteBorder = true, 500); // 250ms out + 250ms in
        ...
        if (Math.abs(deltaY) > 80) {
          ...
          if (showAllOptions) {
            if (deltaY < 0 && isAtBottom) {
              // Swipe hacia arriba Y estamos al final ÔåÆ siguiente encuesta
              console.log('[PollMaximizedView] ­ƒô▒ Cambio a siguiente (al final del scroll)');
              showVoteBorder = false;
              setTimeout(() => {
                transitionX = 0;
                transitionY = 300;
              ...
              // Swipe hacia abajo Y estamos al inicio ÔåÆ encuesta anterior
              console.log('[PollMaximizedView] ­ƒô▒ Cambio a anterior (al inicio del scroll)');
              showVoteBorder = false;
              setTimeout(() => {
                transitionX = 0;
                transitionY = -300;
            ...
            setTimeout(() => {
              transitionX = 0;
              if (deltaY < 0) {
                // Swipe hacia arriba ÔåÆ siguiente encuesta (sale hacia arriba)
                transitionY = 300;
                onSwipeVertical('down');
              } else {
                // Swipe hacia abajo ÔåÆ encuesta anterior (sale hacia abajo)
                transitionY = -300;
                onSwipeVertical('up');
              }
              setTimeout(() => showVoteBorder = true, 500); // 250ms out + 250ms in
  ...
  {#key pollTitle}
    ...
    >
      <div class="poll-header">
        ...
        {:else}
          <textarea
            ...
            placeholder="┬┐Cu├íl es tu pregunta?"
            value={pollTitle}
      ...
      <div class="poll-meta">
        ...
        {#if pollCreatedAt}
          <span class="poll-time">ÔÇó {getRelativeTime(Math.floor((Date.now() - new Date(pollCreatedAt).getTime()) / 60000))}</span>
        {/if}
      </div>
    </div>
  {/key}
    ...
    {#key pollTitle}
      ...
      {#each options as option, idx}
        <button
          ...
          onclick={() => {
            if (readOnly) {
              console.log('[PollMaximizedView] ­ƒù│´©Å Click - Votando:', option.id, option.label);
        ...
        >
          ...
          {#if option.voted}
            <div class="voted-checkmark" style="color: {option.color};">Ô£ô</div>
          {/if}
        </button>
      {/each}
